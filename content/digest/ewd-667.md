...
在这篇短文中，Dijkstra 针对当时很多人热衷的“自然语言编程”泼了一盆冷水，他的核心观点可以总结为一个高度凝练的论述：**编程语言的形式化不是负担而是优势**。

Dijkstra 认为“用自然语言写程序”这种想法并不是问题，问题在其背后更深层的错误认知：认为编程难在机器太笨、编程语言太僵硬，只要机器更“懂人话”，软件开发就会变得轻松。他指出，这恰恰是完全错判了软件开发的本质。软件开发真正做的是理解真实世界的需求然后把需求尽可能**精确**的表达出来，这里“精确”指的是：
- 没有歧义
- 没有隐含假定
- 没有隐藏矛盾
- 能被机械地、可靠地执行
...
这些恰恰是自然语言最不擅长的事情。“自然语言”中的“自然”，实际上指的是，人类在交流时大量依赖上下文、暗示、常识、默认前提、模糊表达，甚至允许说出一些“听上去像那么回事”的话。Dijkstra 直白而尖锐地指出：自然语言的“自然性”，本质上只是“更容易说出荒谬而不自知的话”（*the ease with which we can use them for making statements the nonsense of which is not obvious*）。

Dijkstra 的另一个深刻洞察是，**编程的复杂度并不能简单地在人和机器之间转移**。

很多人会想当然地认为：如果机器更聪明一点，承担更多理解工作，人类就可以少做一点。但 Dijkstra 指出，现实往往相反：接口越复杂，为了跨越接口而进行的解释、沟通、猜测、纠错等工作量反而会明显增加。这就是他一直推崇**窄接口**（*narrow interface*）的原因。好的接口往往是规范而受限的：
- 允许表达的东西有限
- 规则少而清晰
- 双方都知道什么合法、什么不合法
...
今天再读这篇文章，会有一种在点评当下的感觉。过去几十年里，不断被炒作的概念如“低代码”、“无代码”、“只写 prompt 就能开发软件”、“AI 取代程序员”等等，不正和“自然语言编程”异曲同工吗？这些想法与 Dijkstra 当年批评的是同一种冲动：希望把形式化、精确化的负担推给机器。

今天最先进的 LLM 系统也证明了 Dijkstra 是对的：模型在“自由说话”时看起来很聪明，但一旦要真正可靠地完成工作，就必须把输出限制在严格的格式、类型和协议之中，所以我们才会有 JSON schema、领域专用语言（*DSL*）、structured output、function calling 规范等；而一些没有正确配置的 LLM 推理系统可能把 `<think>` 混进 *structured output* 中，从而导致系统崩溃。这正是 Dijkstra 所说的问题：只要接口不够窄、不够形式化，人类觉得“差不多”的东西，对机器来说就可能完全不可用。
...
架构师、系统设计师和程序员从事的核心工作，不是“写某种语法的代码”，而是**把模糊的想法压缩成一个足够严格、足够窄、足够可靠的形式系统**，可能这才是编程（*programming*）一词真正的含义。

现在看来，原文中最有趣的可能是 Dijkstra 在最后补的一段话，他说：

> 我怀疑，那些能够用我们的母语——无论是荷兰语、英语、美式英语、法语、德语，还是斯瓦希里语——来编程的机器，不仅制造起来极其困难，用起来也同样困难。

老爷子可能没想到，这种机器（及其软件）在不到半个世纪之后就做出来了，现在用自然语言“编程”方便又快速，唯一问题是写出来的代码，就像他担心的，充满着各种风险。
