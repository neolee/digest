...
在这篇短文中，Dijkstra 针对当时很多人热衷的“自然语言编程”泼了一盆冷水，而且他的观点不是来自个人喜恶，而是来自严谨的数据式的思考，他的观点可以浓缩成一句话：**编程语言的形式化不是负担而是优势**。
...
很多人会相当然地认为：如果机器更聪明一点，承担更多理解工作，人类就可以少做一点。但 Dijkstra 指出，现实往往相反：接口越复杂，为了跨越接口而进行的解释、沟通、猜测、纠错等工作量反而会明显增加。这就是他一直推崇**窄接口**（*narrow interface*）的原因。好的接口不是“什么都能说”，而是：
- 允许表达的东西有限
- 规则少而清晰
- 双方都知道什么是合法、什么是不合法的
...
甚至今天最先进的 LLM 系统也证明了 Dijkstra 是对的：模型在“自由说话”时看起来很聪明，但一旦要真正可靠地完成工作，就必须把输出限制在严格的格式、类型和协议之中。比如一些没有正确配置 LLM 推理系统会把 `<think>` 混进 *structured output* 中，导致系统崩溃，这正是 Dijkstra 所说的问题：只要接口不够窄、不够形式化，人类觉得“差不多”的东西，对机器来说就可能完全不可用。
